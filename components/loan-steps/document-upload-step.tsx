...
export function DocumentUploadStep({ data, onUpdate, onNext, onPrev, isFirstStep }: DocumentUploadStepProps) {
  ...
  const canProceed = data.propertyImages.length > 0 && data.loanAmount > 0 && data.propertyType
        ...
        <CardContent className="space-y-4">
          ...
          {data.propertyImages.length > 0 && (
            <div className="space-y-2">
              <Label className="text-sm font-medium">ไฟล์ที่อัพโหลด ({data.propertyImages.length})</Label>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) => onUpdate({ propertyValue: Number(e.target.value) })}
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            {data.propertyValue > 0 && data.loanAmount > 0 && (
              <p className="text-xs text-muted-foreground">
                อัตราส่วนสินเชื่อ: {((data.loanAmount / data.propertyValue) * 100).toFixed(1)}%
              </p>
            )}
          </div>
        </CardContent>
